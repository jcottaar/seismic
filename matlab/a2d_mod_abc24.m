function [seis,seis_diff]=a2d_mod_abc24(v,nbc,dx,nt,dt,s,coord,v_diff)
...
%  date:     Sep 26, 2012
%  purpose:  2DTDFD solution to acoustic wave equation with accuracy of 2-4
...
do_gradients = exist('v_diff', 'var');

seis=zeros(nt,numel(coord.gx));
ng=numel(coord.gx);
c1 = -5/2;      % centre weight
c2 =  4/3;      % ±1 neighbours
c3 = -1/12;     % ±2 neighbours
...
if do_gradients
     v_diff = padvel(v_diff,nbc);
    [~,abc_diff]=AbcCoef2D(v,nbc,dx,v_diff);
    kappa_diff = abc_diff*dt;   
    alpha_diff = 2*(dt/dx).^2*(v.*v_diff);
    temp1_diff = 2*c1*alpha_diff-kappa_diff;
    temp2_diff = -kappa_diff;
    beta_dt_diff = 2*(v.*v_diff)*dt.^2;
end
...
inds = gpuArray(sub2ind(size(p1), igz, igx));

if do_gradients
    p1_diff = 0*p1; p0_diff=0*p0;
end
...
for it=1:nt
    ...
    if do_gradients
        lapg_diff = imfilter(p1_diff, hg, 'circular', 'conv');
        p_diff = temp1_diff.*p1 + temp1.*p1_diff - temp2_diff.*p0 - temp2.*p0_diff + alpha_diff.*lapg + alpha.*lapg_diff;
    end
   
    
    p(isz,isx) = p(isz,isx) + beta_dt(isz,isx) * s(it);
    if do_gradients
        p_diff(isz,isx) = p_diff(isz,isx) + beta_dt_diff(isz,isx) * s(it);
    end
    seis(it,:) = p(inds);
    if do_gradients
        seis_diff(it,:) = p_diff(inds);
    end
    p0=p1;
    p1=p;
    if do_gradients
