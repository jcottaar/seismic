...
def test_stuff_on_one_case(d, expected_match, test_reference_mode=False):
    ...
    diff_result = seis_forward.prep_run_diff(d.velocity, cp.reshape(offset_vecs[:,:-1],(N_test,70,70)), offset_vecs[:,-1])
    for ii in range(N_test):
        for jj in range(len(offset_result)):
    ...
    base_result, p_complete = seis_forward.vel_to_seis(d.velocity, d.seismogram, return_p_complete_list=True)    
    ...
    diff_result = seis_forward.vel_to_seis_diff(d.velocity, d.seismogram, p_complete, offset_vecs)
